...
async def load_ngrams_from_binary(path: str) -> str:
    global ngrams
    if not os.path.exists(path):
        with open(path, "wb") as f:
            pickle.dump({}, f)
        return f"[PY] Ngrams file {path} did not exist, created empty file."

    with open(path, 'rb') as f:
        ngrams = pickle.load(f)

    return f"[PY] Ngrams successfully loaded from {path}!"


async def save_ngrams_to_binary(path: str) -> str:
    with open(path, "wb") as f:
        pickle.dump(ngrams, f)

    return f"[PY] Ngrams successfully saved to {path}!"
...
async def build_ngrams(split_strategy: str, character_count: int, optional_text: str | None = None) -> str:
    ...
    if len(words) == 1:
        ...
        if word not in ngrams:
            ngrams[word] = []
        return f"Single {split_strategy} added to ngrams (chat)"
    ...
    return f"Ngrams built by {character_count} characters (chat)"
...
def split_words_with_spaces(text: str) -> list[str]:
    ...
    for char in text:
        current_chunk += char
        if char.isspace() and current_chunk.strip():  # If we hit a space and have non-space chars
